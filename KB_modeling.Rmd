...
playoff_success <- read.csv("MLB_Postseason_Success.csv", stringsAsFactors = FALSE) %>%
  mutate_all(na_if,"") %>%
  replace(is.na(.), "No Playoffs")

#combining new palyoff dataset with dat
dat <- left_join(dat, playoff_success, by = c("team", "year")) %>%
  select(-c(DivWin, WCWin, LgWin, WSWin)) # deselecting unnecessary variables
...
annual_means <- lm(ghits_e1 ~ odds + winpct + attendance + payroll + lag(ghits_e1, 1), data = by_year) ## this should be an AR model, annual
...
final_model <- 
  dat  %>%
  lm(ghits_e2 ~ phase + is_playoffs:playoffs + lag(ghits_e2, 1), data = .) ## needs to be AR model instead, monthly
